...
- **Безопасность кред S3 в сообщении:** креды летят в Kafka-payload → request-топик должен быть с
  ACL/шифрованием, креды короткоживущие и не должны попадать в логи агента. Альтернатива на будущее —
  агент возвращает presigned URL вместо ключа, чтобы клиенту не нужны были read-креды.
```

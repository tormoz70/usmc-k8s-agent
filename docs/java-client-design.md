...
1. **Транспорт generic-операций:** подтверждаем гибрид (HTTP-туннель Вариант 1 для CRUD + типизированные
   команды для заданий) или хотим только типизированный командный DSL (Вариант 2)? Подробное объяснение
   того, как это работает, — в [java-client-transport-options.md](java-client-transport-options.md).

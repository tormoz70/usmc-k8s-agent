...
- Поддерживаются две реализации агента с общим command-контрактом: **v1** (Deployments + `GetLogs`) и **v2** (те же Deployments + DaemonSet с чтением `/var/log/pods`). В PROD на кластере ставится ровно один вариант. Подробности: [`agent-v1-v2.md`](./agent-v1-v2.md).
